...
final class ConfigurationServer extends AbstractServer
{
    ...
    public function methods(): array
    {
        ...
        if ($methods === null) {
            $methodsPath = mb_rtrim((string) config('rpc.paths.methods'), '/');
            ...
            return collect(File::allFiles($methodsPath))
                ...
                ->filter(fn ($class): bool => class_exists($class) && in_array(MethodInterface::class, (array) class_implements($class), true))
                ->values()
                ->toArray();
        }
